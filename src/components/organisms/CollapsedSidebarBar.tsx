...
export function CollapsedSidebarBar({ onExpand }: { onExpand: () => void }) {
  ...
  return (
    <div
      ...
      style={{ backgroundColor: "var(--sidebar-bg)", color: "var(--text-primary)", boxShadow: "0 4px 12px rgba(0, 0, 0, 0.15), 0 0 1px rgba(0, 0, 0, 0.1)", minHeight: 48 }}
    ...
    >
      <div className="relative flex items-center">
        ...
        >
          <span className="flex h-9 w-9 items-center justify-center"><FigmaLogoIcon /></span>
      ...
      <button
        ...
        onClick={onExpand}
      >
        <span className="min-w-0 flex-1 truncate text-left font-medium" style={{ fontSize: "var(--sidebar-font-size)" }}>Antital</span>
        <span className="flex h-9 w-9 shrink-0 items-center justify-center" aria-hidden><MinimizeUIIcon /></span>
      </button>
    </div>
  );
}
